...
@pytest.mark.asyncio
async def test_update_user_role_raise_401_invalid_permissions_dev_cant_change_role(
    client: AsyncClient,
    mock_user,
    admin_url,
    user_agent,
...
@pytest.mark.asyncio
async def test_update_post_success_raise_401_invalid_permissions(
    client: AsyncClient,
    admin_url: str,
    user_agent: str,
    mock_user_out_inserted: UserOut,
...
async def test_update_post_raise_500_unable_update_entity_error(
    ...
    with patch.multiple(
        PostsRepository,
        get_post_by_id=AsyncMock(return_value=mock_post_inserted),
        update_post=AsyncMock(side_effect=UnableUpdateEntity),
    ):
        result = await client.put(
            f"{admin_url}/posts/{mock_post_inserted.id}",
            headers={
...
async def test_update_post_raise_500_generic_error(
    ...
    with patch.multiple(
        PostsRepository,
        get_post_by_id=AsyncMock(return_value=mock_post_inserted),
        update_post=AsyncMock(side_effect=GenericError),
    ):
        result = await client.put(
            f"{admin_url}/posts/{mock_post_inserted.id}",
            headers={
...
@pytest.mark.asyncio
async def test_delete_post_success_as_admin(
    client: AsyncClient,
    admin_url: str,
    user_agent: str,
    mock_user_out_inserted: UserOut,
...
@pytest.mark.asyncio
async def test_delete_comment_raise_401_unauthorized(
    client: AsyncClient,
    admin_url: str,
    comments_url: str,
    user_agent: str,
...
@pytest.mark.asyncio
async def test_delete_comment_raise_404_not_found(
    client: AsyncClient,
    admin_url: str,
    comments_url: str,
    user_agent: str,
